...
### Added
- **TCP ping отсев** — новый этап перед проверкой конфигов. До запуска ядра checker делает обычный TCP-коннект до `host:port` каждой ссылки и выкидывает те, что не принимают соединение. Ядро больше не поднимается ради заведомо мёртвых прокси. Идея: Telegram **@loliconshik**.
- Отсев **выключен по умолчанию** (`tcp_ping.enabled = false`): он судит прокси по TCP-коннекту с машины пользователя и на зажатом канале может выкинуть живые ссылки. Включается флагом `--tcp-ping`, пунктом меню или ключом в конфиге, так что поведение проверки без явного включения не меняется.
- Блок `tcp_ping` в `config.json`: `enabled`, `timeout`, `concurrency`, `retries`, `max_latency_ms`, `skip_udp`, `resolve_timeout`, `max_ips_per_host`, `report_file`.
- Пункт меню `Настройки → TCP ping отсев`: тумблер, таймаут, число одновременных коннектов, ретраи, порог TCP RTT, поведение с UDP-протоколами и файл отчёта. Состояние отсева выведено в статус-панель.
- Пункт меню `Проверка → TCP ping отсев`: прогнать файл или ссылку только через TCP-отсев и сохранить живые ссылки, без запуска ядра.
- CLI: `--tcp-ping` / `--no-tcp-ping`, `--tcp-ping-only`, `--tcp-ping-timeout`, `--tcp-ping-concurrency`, `--tcp-ping-retries`, `--tcp-ping-max-ms`.
- `--tcp-ping-max-ms` даёт второй, более дешёвый порог задержки: TCP RTT до сервера отсекается ещё до того, как ядро потратит время на реальный запрос через прокси.
- Опциональный `report_file` со списком живых `ip:port` и их RTT.

### Changed
- Отсев асинхронный (`asyncio`, без новых зависимостей): пул воркеров вместо задачи на каждую ссылку, поэтому память не зависит от размера списка. DNS резолвится один раз на хост и кешируется на весь прогон.
- Ретраится только `timeout`. `refused` и `unreachable` — окончательный ответ хоста, повторять их бессмысленно.
- UDP/QUIC-протоколы (`hysteria`, `hysteria2`, `tuic`, `wireguard`, `shadowquic`, `masque`) по умолчанию проходят мимо отсева: TCP-коннект на их порт ничего не доказывает и выбросил бы живые прокси. Отключается через `skip_udp`.
- Ссылки без распознанного `host:port` отсев не трогает — он удаляет только то, что реально проверил.
- Дефолтный `timeout` — `3.0` сек. Ниже `2.5` ставить не стоит: на Windows `ConnectEx` отдаёт `refused` только через ~2 сек (SYN-ретрай), и отказ начал бы читаться как timeout с лишним ретраем.
