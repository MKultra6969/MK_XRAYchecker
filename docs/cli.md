...
Быстрый TCP-коннект до `host:port` перед запуском ядра. По умолчанию отсев выключен. Все флаги перекрывают блок `tcp_ping` из `config.json` на один запуск.

| Аргумент | Описание |
| :--- | :--- |
| `--tcp-ping` | Включить отсев на этот запуск (v1.10.0) |
| `--no-tcp-ping` | Отключить отсев, даже если в конфиге он включён (v1.10.0) |
