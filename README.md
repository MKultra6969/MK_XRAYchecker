...
*   **TCP ping отсев:** Перед запуском ядра checker делает обычный TCP-коннект до `host:port` и выкидывает мёртвые ссылки. Ядро не тратит время на то, что и так не отвечает. По умолчанию **выключен**, включается флагом или в меню. Настраивается целиком: таймаут, потоки, ретраи, порог RTT.
...
**По умолчанию отсев выключен.** Он судит прокси по TCP-коннекту с твоей машины, и на кривом или зажатом канале может выкинуть живые ссылки — поэтому включается осознанно.

```bash
# включить на один запуск
python v2rayChecker.py -f "list.txt" --tcp-ping

# подкрутить под свой канал
python v2rayChecker.py -f "list.txt" --tcp-ping --tcp-ping-timeout 2 --tcp-ping-concurrency 1000

# выкинуть ещё и всё, до чего TCP RTT больше 300 мс
python v2rayChecker.py -f "list.txt" --tcp-ping --tcp-ping-max-ms 300
```

Включить навсегда — `Настройки → TCP ping отсев → 1` в меню или `"enabled": true` в блоке `tcp_ping`. `--tcp-ping-only` работает всегда: это явный запуск инструмента, тумблер ему не нужен.
